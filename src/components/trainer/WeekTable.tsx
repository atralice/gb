...
export default function WeekTable({ data, onDayClick }: WeekTableProps) {
  return (
    <div>
    ...
    <>
      <tr
        className={`bg-slate-100 border-y border-slate-200 ${
          onDayClick ? "cursor-pointer hover:bg-slate-200" : ""
        }`}
        ...
        exercise.sets.map((set, setIndex) => (
          <tr key={set.id} className="border-b border-slate-100 last:border-0">
            ...
            <td className="px-3 py-2 text-center">
              <span className="text-sm text-slate-600">{set.reps ?? "—"}</span>
            </td>
            <td className="px-3 py-2 text-center">
